...
import ThumbnailFrame from "@/components/thumbnail-frame"
...
export default function ProjectsGallery({ initialProjects = [], initialShowCount = 6 }: Props) {
  ...
  return (
    <div className="space-y-6">
        ...
        {toShow.map((p, idx) => (
          ...
          >
            <Card className="bg-white/5 backdrop-blur-xl border-white/10 hover:bg-white/10 transition-all h-full overflow-hidden">
              <ThumbnailFrame src={p.image} alt={p.title}>
                {p.award && (
                ...
                <div className="flex gap-4">
                  {p.github && (
                    <Button
                      variant="outline"
                      size="sm"
                      className="border-cyan-400/40 text-cyan-300 hover:bg-cyan-500 hover:text-black transition-colors bg-transparent"
                  ...
                  {p.demo && (
                    <Button
                      variant="outline"
                      size="sm"
                      className="border-purple-400/40 text-purple-300 hover:bg-purple-500 hover:text-black transition-colors bg-transparent"
      ...
      {/* Empty state */}
      {filtered.length === 0 && <div className="text-center text-white/70 py-10">No projects match your filters.</div>}
    </div>
  )
}
